...
// Types pour les sections Background
...
export default function AboutContent() {
    // useLang : lit la langue depuis l'URL (/fr/about → "fr")
    const lang = useLang();
    const t = getDictionary(lang);

    // Les données Background dépendent de la langue courante
    const backgroundData: BgSection[] = [
        {
            id: "01",
            category: t.about.myStoryCategory,
            type: "text",
            image: `${BASE_PATH}/profil.jpg`,
            content: t.about.myStoryContent,
        },
        {
            id: "02",
            category: t.about.certificationsCategory,
            type: "table",
            headers: t.about.certificationsHeaders,
            items: t.about.certificationsItems,
        },
        {
            id: "03",
            category: t.about.recognitionsCategory,
            type: "list",
            items: t.about.recognitionsItems,
        },
    ];
    ...
    }, [lang]); // lang en dépendance : relance les animations si la langue change
    ...
    return (
        <main className="relative overflow-hidden">
            ...
            {/* Section 2: Citation */}
            <TextParagraph subtitle={t.about.philosophySubtitle} paragraphs={t.about.philosophyLines} />
            ...
            <section ref={backgroundSectionRef} className="flex flex-col items-center justify-start min-h-screen p-5 w-full pt-40 mb-20 md:mb-80 mt-0 md:mt-60">
                <div className="w-full flex flex-col items-center px-1 md:px-2">
                    <div className="flex flex-row justify-between items-end w-full z-10 mb-10 md:mb-16 whitespace-nowrap px-0.5 md:px-1">
                        <h1 className="bg-title-my font-sans text-[clamp(5.5rem,14vw,7.5rem)] md:text-[20rem] lg:text-[24rem] leading-none uppercase tracking-wide -ml-5 md:-ml-10">{t.about.backgroundTitle0}</h1>
                        <h1 className="font-script text-[clamp(3rem,8vw,5.5rem)] md:text-[10rem] lg:text-[13rem] tracking-tight leading-none md:pr-10">
                            <span className="text-ink">{t.about.backgroundTitle}</span>
                        </h1>
                    </div>
                                                ...
                                                {section.type === "text" && section.content && (
                                                    <div className="space-y-5 md:space-y-8">
                                                        {section.content.map((para, i) => (
                                                            <p key={i} className="text-base md:text-xl lg:text-2xl font-medium leading-[1.25]">{para}</p>
                                                        ))}
                                                    </div>
                                                )}
                                                {section.type === "table" && (
            ...
            <PageEndSection
                ref={ctaSectionRef}
                title={t.shared.pageEndTitle}
                ctaLabel={t.shared.pageEndCta}
            />
        </main>
    );
}
